...
**以下为硕士期间的主要课程（纯实践课程及活动，如实习、毕业项目等不计入）：**

* **Data Management:** 本课程主要介绍了数据库的相关知识，介绍并使用了常用相关型数据库与非相关型数据库，包括MongoDB和Neo4j，进一步还介绍了数据仓库、数据融合、数据质量，以及Hadoop等架构
* **Data Visulization**本课程学习了常用的数据可视化技术和工具，并聚焦与数据可视化的设计原则，如正确的根据数据类型选择合适的可视化方法，并且避免可能造成的误导的可视化
* **Juridical and Social Issues in Information Society:**这门课分为两个部分，第一个部分主要讨论了信息社会的常识、问题、理论等等，包括科学、技术与社会的关系；自动化的概念及其影响，并且详细介绍了一些社会学理论，最后还特别讨论了AI技术对社会与伦理的影响。第二部分讲述了数据与人工智能相关的法律问题，包括不同社会目前对大数据技术与AI的处理态度与司法体系、监管方式等。
* **Machine Learning:**本课程主要学习了3部分知识，分别是常用的分类技术、聚类技术，以及关联分析。课程深入浅出，每个领域都不仅仅介绍技术，而尤其重视技术的适用场景和完成技术之后的评估与分析步骤，包括模型的效果和有效性（如零假设等手段）使模型选择和分析建立在合理的统计与实际意义之上。
